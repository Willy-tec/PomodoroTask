...
<template>
    <div class="Tasklist">
        <div class="Tasklist_container">
            <TaskCard v-for="task in tasklist.tasks.filter(e => !e.done).sort((a, b) => a.id - b.id)" :id=task.id
                :done=task.done :description=task.description @toggleDone="task.done = !task.done"
                @deleteCard="deleteCard(task.id)"></TaskCard>
            <TaskCard v-for="task in tasklist.tasks.filter(e => e.done).sort((a, b) => a.id - b.id)" :id=task.id
                :done=task.done :description=task.description @toggleDone="task.done = !task.done"
                @deleteCard="deleteCard(task.id)"></TaskCard>
        </div>
        <div class="tasklist_button_container">
            ...
            <input v-if="addingTask" v-model="taskValue" type="text" @change="addTask(taskValue)"
                @focusout="addingTask = false" />
        </div>
    </div>
</template>
